...
func TestBackendImplementation_NewRequest(t *testing.T) {
	const privPEM = `
-----BEGIN RSA PRIVATE KEY-----
MIIEowIBAAKCAQEA4f5wg5l2hKsTeNem/V41fGnJm6gOdrj8ym3rFkEU/wT8RDtn
SgFEZOQpHEgQ7JL38xUfU0Y3g6aYw9QT0hJ7mCpz9Er5qLaMXJwZxzHzAahlfA0i
...
-----END PUBLIC KEY-----`
	...
	cfg, err := NewConfig("test", "test-key", OptionAppSwitchKeyPair([]byte(pubPEM), []byte(privPEM)))
	assert.NoError(t, err)
	...
	req, err := backend.NewRequest("", "", buf, "")
	assert.NoError(t, err)

	assert.Empty(t, req.Header.Get("Test-mode"))
	assert.Equal(t, "test-key", req.Header.Get("Ocp-Apim-Subscription-Key"))
}

func TestBackendImplementation_NewRequestTestMode(t *testing.T) {
	const privPEM = `
-----BEGIN RSA PRIVATE KEY-----
MIIEowIBAAKCAQEA4f5wg5l2hKsTeNem/V41fGnJm6gOdrj8ym3rFkEU/wT8RDtn
SgFEZOQpHEgQ7JL38xUfU0Y3g6aYw9QT0hJ7mCpz9Er5qLaMXJwZxzHzAahlfA0i
...
-----END PUBLIC KEY-----`
	...
	assert.Equal(t, "true", req.Header.Get("Test-mode"))
}
